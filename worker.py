...
record_id_str = os.getenv("RECORD_IDS")  # Only one record ID now
...
if record:
    try:
        ...
        try:
            ...
            deserialized_payload = json.loads(decoded_str)

            logger.info(f"Deserialized payload for {record['_id']}: {deserialized_payload}")
        except json.JSONDecodeError as e:
            logger.error(f"Failed to deserialize payload for {record['_id']}: {e}")
            raise
